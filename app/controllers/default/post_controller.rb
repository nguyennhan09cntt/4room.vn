class Default::PostController < DefaultController	
    ...
    @posts = Post.select(:id, :uid, :facebook_id,:created_at).where('site_id = :site_id', {site_id: @site[:id]}).paginate(:page => params[:page], :per_page => 6).reorder("created_at DESC, id DESC")
	...
	def show
    	...
    	@post = Post.where('uid = :post_id', { post_id: post_id }).first
    else
